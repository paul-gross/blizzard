...
def test_an_open_lease_reads_local_and_is_never_asked_of_the_hub(tmp_path: Path) -> None:
    ...
    archived = FakeArchivedTranscriptRepository(
        {
            _KEY: ArchivedTranscript(
                status="found", turns=[_hub_turn("should never be seen")], truncated=False, dropped=0
            )
        }
    )
...
@pytest.mark.unit
def test_a_closed_lease_with_hub_segments_serves_them_and_reports_the_dropped_count(tmp_path: Path) -> None:
    store = make_store(f"sqlite:///{tmp_path / 'runner.db'}")
    _closed_lease(store)
    local = FakeTranscriptRepository(
        {
    ...
    archived = FakeArchivedTranscriptRepository(
        {_KEY: ArchivedTranscript(status="found", turns=[_hub_turn("from the hub")], truncated=False, dropped=3)}
    )
...
@pytest.mark.unit
def test_a_hub_read_that_drops_nothing_reports_zero(tmp_path: Path) -> None:
    store = make_store(f"sqlite:///{tmp_path / 'runner.db'}")
    _closed_lease(store)
    archived = FakeArchivedTranscriptRepository(
        {_KEY: ArchivedTranscript(status="found", turns=[_hub_turn("from the hub")], truncated=False, dropped=0)}
    )
...
def test_a_closed_lease_the_hub_refuses_falls_back_to_local(tmp_path: Path) -> None:
    ...
    archived = FakeArchivedTranscriptRepository(
        {_KEY: ArchivedTranscript(status="refused", turns=[], truncated=False, dropped=0)}
    )
...
def test_a_closed_lease_the_hub_is_unreachable_but_local_still_answers_falls_back_quietly(tmp_path: Path) -> None:
    ...
    archived = FakeArchivedTranscriptRepository(
        {_KEY: ArchivedTranscript(status="unreachable", turns=[], truncated=False, dropped=0)}
    )
