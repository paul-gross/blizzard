...
def test_distinct_kinds_at_the_same_generation_coexist() -> None:
    ...
    store = make_store("sqlite://")
    store.record_boundary_open(
        lease_id="lease_1", chunk_id="ch_1", node_id="nd_build", epoch=1, generation=2, kind="nudge",
        start_position="pos-a", opened_at=_T0,
    )
...
def test_open_boundaries_for_lease_excludes_closed_rows() -> None:
    store = make_store("sqlite://")
    store.record_boundary_open(
        lease_id="lease_1", chunk_id="ch_1", node_id="nd_build", epoch=1, generation=1, kind="spawn",
        start_position=None, opened_at=_T0,
    )
    store.record_boundary_open(
        lease_id="lease_1", chunk_id="ch_1", node_id="nd_build", epoch=1, generation=1, kind="judge",
        start_position="pos-judge", opened_at=_T0,
    )
...
def test_close_boundaries_for_lease_leaves_other_leases_untouched() -> None:
    ...
    store.record_boundary_open(
        lease_id="lease_2", chunk_id="ch_2", node_id="nd_build", epoch=1, generation=1, kind="spawn",
        start_position=None, opened_at=_T0,
    )
