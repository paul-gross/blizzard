...
This guard closes that gap durably: for every packaged ``*.yaml`` graph, for every runner
node declaring a ``produces:`` name, it asserts the node's inlined prompt text (the main
prompt and the judgement prompt) names ``attach --name <that-exact-name>``. A future prompt
edit that drops or mistypes the attach instruction fails here rather than shipping green.
...
def _packaged_graphs() -> list[tuple[str, GraphDoc]]:
    """(filename, loaded+inlined GraphDoc) for every packaged graph YAML."""
    return [(path.name, load_graph_doc(path)) for path in sorted(_GRAPHS_DIR.glob("*.yaml"))]


def _asset_producing_worker_nodes() -> list[tuple[str, NodeDoc, str]]:
    """(graph filename, node, produces-name) for every runner node that declares an asset.
    ...
    for filename, doc in _packaged_graphs():
        for node in doc.nodes:
            if node.executor is Executor.RUNNER:
                for name in node.produces:
                    triples.append((filename, node, name))
    return triples
...
def test_packaged_graphs_declare_at_least_one_asset_producing_node() -> None:
    ...
    ``review-findings`` asset in both packaged graphs.
    """
...
@pytest.mark.parametrize(
    ("filename", "node", "name"),
    [pytest.param(f, n, name, id=f"{f}:{n.name}:{name}") for f, n, name in _asset_producing_worker_nodes()],
)
def test_asset_producing_node_prompt_names_the_attach_cli(filename: str, node: NodeDoc, name: str) -> None:
    ...
    assert needle in text, (
        f"{filename}: node {node.name!r} declares produces {name!r} but its prompt text "
