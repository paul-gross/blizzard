"""The selftest-results migration (blizzard#438, Phase 2 — component tier): applies from
...
def test_selftest_result_tables_exist_after_a_fresh_migration_to_head(tmp_path: Path) -> None:
    ...
    assert "selftest_result_checks" in tables


def test_selftest_result_tables_survive_a_downgrade_upgrade_roundtrip(tmp_path: Path) -> None:
    ...
    assert "selftest_result_checks" not in tables
