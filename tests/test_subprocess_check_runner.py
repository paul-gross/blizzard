...
def test_a_zero_exit_is_a_pass_and_captures_output() -> None:
    outcome = SubprocessCheckRunner(worker_env=_EMPTY).run("echo hello", cwd=".", timeout=30)
    assert outcome.passed is True
    assert "hello" in outcome.output_tail


def test_a_nonzero_exit_is_a_red_check() -> None:
    outcome = SubprocessCheckRunner(worker_env=_EMPTY).run("echo boom >&2; exit 3", cwd=".", timeout=30)
...
def test_a_timeout_is_a_red_check_not_a_raise() -> None:
    outcome = SubprocessCheckRunner(worker_env=_EMPTY).run("sleep 5", cwd=".", timeout=1)
    assert outcome.passed is False
...
def test_runs_in_the_given_cwd(tmp_path: Path) -> None:
    ...
    outcome = SubprocessCheckRunner(worker_env=_EMPTY).run("ls marker.txt", cwd=str(tmp_path), timeout=30)
    assert outcome.passed is True
...
def test_the_child_env_excludes_daemon_secrets(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    outcome = SubprocessCheckRunner(worker_env=_EMPTY).run("echo token=[${BZ_HUB_TOKEN:-absent}]", cwd=".", timeout=30)
    assert outcome.passed is True
...
def test_an_operator_passthrough_var_reaches_the_child(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    runner = SubprocessCheckRunner(worker_env=AllowlistedEnv.of(("MY_TOOLCHAIN_QUIRK",)))
    outcome = runner.run("echo quirk=[${MY_TOOLCHAIN_QUIRK:-absent}]", cwd=".", timeout=30)
    assert "quirk=[on]" in outcome.output_tail


def test_a_path_prepend_entry_leads_the_childs_path() -> None:
    runner = SubprocessCheckRunner(worker_env=AllowlistedEnv.of((), path_prepend=("/opt/mise/shims",)))
    outcome = runner.run("echo path=[$PATH]", cwd=".", timeout=30)
    assert outcome.passed is True
    assert "path=[/opt/mise/shims:" in outcome.output_tail


def test_the_output_tail_is_bounded() -> None:
    # Emit far more than the tail ceiling; the tail is clipped with an elision marker.
    outcome = SubprocessCheckRunner(worker_env=_EMPTY).run(
        "for i in $(seq 1 5000); do echo linexxxxxx$i; done", cwd=".", timeout=30
    )
    assert outcome.passed is True
