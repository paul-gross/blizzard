...
from blizzard.runner.harness.internal.offline_compatibility import DEFAULT_CORPUS_ROOT
...
def test_opencode_health_probe_declares_the_pinned_versions_absences() -> None:
    ...
    manifest = _manifest()
    assert probe.supported_version() == ADMITTED_OPENCODE_VERSIONS == frozenset({_AN_ADMITTED_OPENCODE_VERSION})
...
def test_opencode_health_probe_construction_survives_a_missing_corpus_manifest(tmp_path: Path) -> None:
    ...
    assert probe.supported_version() == ADMITTED_OPENCODE_VERSIONS
    assert probe.declared_degradations() == ()
