...
def test_backfilled_session_resumes_on_an_answer(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    harness = FakeHarness(
        handle=WorkerHandle(session_id="sess-resume-old", pid=200, process_start_time="start-200", pgid=200),
        verdict=None,
    )
...
def test_backfilled_session_is_judged_after_exit(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    harness = FakeHarness(
        handle=WorkerHandle(session_id="sess-judge-old", pid=300, process_start_time="start-300", pgid=300),
        verdict="pass",
    )
...
def test_backfilled_session_takeover_opens_under_claude_code(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    harness = FakeHarness(
        handle=WorkerHandle(session_id="sess-takeover-old", pid=500, process_start_time="start-500", pgid=500),
        verdict=None,
    )
