...
def test_peek_reads_facts_in_bulk_and_never_per_chunk(tmp_path: Path, path: str, promote: bool) -> None:
    ...
    # The candidate list resolves via `load_all_statuses` (blizzard#bulk-read-seams, no
    # longer `load_all_facts` — the list route only needs each chunk's derived status),
    # while the blocked marking (issue #457) still needs each chunk's full fact history.
