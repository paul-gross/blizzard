...
def test_work_items_tables_survive_migration_roundtrip(tmp_path: Path) -> None:
    ...
    def _has_tables() -> bool:
        ...
        try:
            ...
            return "work_items" in names and "work_item_sequence" in names
        finally:
            engine.dispose()

    assert _has_tables()

    runner.downgrade("20260819_0900_chunk_restarts")
...
def test_scopes_and_routines_tables_survive_migration_roundtrip(tmp_path: Path) -> None:
    ...
    config = hub_runtime.init_environment(tmp_path)  # upgrades to head
    runner = hub_runtime.migration_runner(config)
    tables = ("scopes", "scope_lifecycle_facts", "routines")
    ...
    runner.downgrade("20260826_0930_close_intents_backfill")
...
_HISTORICAL_RESHAPES: list[tuple[str, str, str, tuple[str, ...]] | tuple[str, str, str, tuple[str, ...], str]] = [
    ...
    # `routines` has no entry here: its create-table revision (20260828_1000_scopes_and_
    # routines.py) imports `routines` live off `schema.py` rather than a frozen local
    # snapshot (`default_model`/`default_effort` are absent from this list for the same
    # reason), so `default_harnesses` is already present from that revision onward.
