...
request; ``PACKAGED`` is monkeypatched to a throwaway root so the route is exercised without
depending on what is actually shipped."""
...
import blizzard.hub.api.system_artifacts as system_artifacts_api
...
def test_list_serves_the_full_published_set(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(system_artifacts_api, "PACKAGED", _packaged(tmp_path / "packaged"))
...
def test_list_is_empty_when_nothing_is_published(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(system_artifacts_api, "PACKAGED", PackagedSystemArtifacts(tmp_path / "empty"))
    hub = build_hub(tmp_path)
...
def test_get_resolves_a_slash_bearing_name(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(system_artifacts_api, "PACKAGED", _packaged(tmp_path / "packaged"))
    hub = build_hub(tmp_path)
...
def test_get_404s_for_an_unpublished_name(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(system_artifacts_api, "PACKAGED", _packaged(tmp_path / "packaged"))
    hub = build_hub(tmp_path)
