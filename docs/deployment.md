...
Unlike `annotate`/`close`, item mutation (`POST`/`GET`/`PATCH`/`DELETE` under `/api/work-sources/{source}/items`) has no
`[[work_source]]` knob to opt a configured source into: it is served only by the built-in `hub` source, whose own store
is a hub-owned item's system of record. An operator `PATCH`/`POST` against a configured forge source refuses with a
409 naming it — by design, not by a missing flag a future block could set.
