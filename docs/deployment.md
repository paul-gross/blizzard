...
it can succeed even against an escalation carrying neither, and it refuses with
`ChunkNotTakeable` only when that underlying session genuinely isn't there. Resolving
an escalation the verb refuses on means acting on the chunk directly (reading its
bounce history or migration guidance) and requeuing, not taking anything over.
