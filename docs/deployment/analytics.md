...
<instant> [--until <instant>]` — the same rows the matching `blizzard hub analytics summary` dataset serves for the same
window, gated on the chunk's own run context rather than an operator credential. `--since` is required here (unlike the
operator's own optional one): a worker call carries no separate fleet-wide-scan deferral to lean on. Neither verb takes
