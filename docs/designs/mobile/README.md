...
Static HTML mockup sheets for the mobile variant of the hub/runner web dashboards. Open either file directly in a
browser; each renders phone frames beside annotation cards that name the flow decisions and the existing `fleet`
components each screen reuses versus what is net-new.
...
Cross-sheet scoping notes, in build order: the push-notification channel comes first (four of the eight flows are
notification-born), then the shells; the only genuinely new backend plumbing is a windowed digest query, the spend/redo
alarm rule, a forge diffstat read, a runner-transcript proxy route for the hub, and the heartbeat-recovery notification.
Graphs authoring stays desktop-only.
