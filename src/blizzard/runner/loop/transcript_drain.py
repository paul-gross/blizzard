...
#: The reason `_deliver` marks on a hub-cap-rejected record (review F6) — the hub's own
#: cap adjudication, distinct from the runner's own reasons in `transcript_pump.py`.
...
class TranscriptDrain:
    ...
    def run(self) -> None:
        ...
        # F17) — a second, loop-level guard would be dead code below this line's cap.
    ...
    def _deliver(self, delta: BufferedTranscriptDelta) -> bool:
        ...
        if delta.seq in ack.capped:
            ...
            # Never silent (review F6) — the same segment-field/fact-lane pair the pump's
            # own truncation paths already use, not just the log line above.
    ...
    def _render(self, delta: BufferedTranscriptDelta) -> TranscriptSegmentRecord:
        ...
        :class:`TranscriptPump`. A final marker's is deliberately minimal (review F8) —
        every field it needs is already frozen on the ledger row, read straight from
        there rather than a second hand-built copy."""
        if not delta.final:
