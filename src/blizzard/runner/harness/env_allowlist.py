...
Lives here (not inside the ``claude_code_adapter``) because more than one seam builds
a child env from it, and a single owner is what keeps them from drifting
(``bzh:one-owner``).
"""
...
# `ANTHROPIC_MODEL` and its family are deliberately ABSENT and must stay absent — here
# and in `[worker] env_passthrough` — because they override the model a resumed session
# restores, which the mint-only `--model` contract rests on (issue #144; pinned by
