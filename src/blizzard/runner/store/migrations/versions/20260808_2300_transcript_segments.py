...
transcript_segments = sa.Table(
    ...
    sa.Column("shipping_stopped_reason", sa.String, nullable=True),
...
# Declared once, on the frozen table above: `table.create` below carries it, and the
# explicit create/drop stays `checkfirst` for a re-applied revision.
_CHUNK_INDEX = sa.Index(_INDEX, transcript_segments.c.chunk_id)
...
def upgrade() -> None:
    ...
    _CHUNK_INDEX.create(bind, checkfirst=True)


def downgrade() -> None:
    bind = op.get_bind()
    _CHUNK_INDEX.drop(bind, checkfirst=True)
