...
class ChunkDetailView(BaseModel):
    ...
    proxy serves whole, carrying its **transition history** and inline **artifact store**. ``escalation``
    stays hub-only: the runner already serves its own, differently-shaped one on ``GET /api/escalations``,
    and one app cannot carry two same-named schemas without mangling both."""
