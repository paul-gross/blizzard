...
class ChunkSummary(BaseModel):
    ...
    # Carried independently of ``status``; see BlockedView.
...
class ChunkDetail(BaseModel):
    ...
    # The chunk's cross-graph migration steps, oldest first — interleaves with ``history`` by
    # ``recorded_at``. Empty for a single-graph chunk.
    migrations: list[MigrationView] = []
    # The chunk's operator restarts, oldest first — who moved the chunk, from where, and to what.
