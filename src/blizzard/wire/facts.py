...
* The **batched store-and-forward push** — :class:`RunnerFactBatch` to ``POST
  ...
  replay after a lost ack or an outage backlog drain).
...
# The ask/answer pair the runner forwards up: question.asked opens the ask,
# answer.delivered records that the resume-with-answer ran.
QUESTION_ASKED = "question.asked"
ANSWER_DELIVERED = "answer.delivered"
# The runner's *own* brake, reported upward (issue #43). Named apart from the hub's
# `runner.paused` / `runner.resumed` on purpose: they are two concepts, not two spellings
# — this one is the runner declining to claim, set machine-locally and true even when the
# hub never heard about it. Runner-scoped: no chunk_id, no lease_id.
...
# stored aggregate. Payload: {chunk_id, node_id, epoch, kind, model, input_tokens,
...
# stall. Payload (open, like `usage.recorded`): {severity (info|warning|critical),
...
# (issue #218) — an advisory fact, never one a status derives from.
# Runner-scoped: no chunk_id, no lease_id. Payload: {sampled_at,
