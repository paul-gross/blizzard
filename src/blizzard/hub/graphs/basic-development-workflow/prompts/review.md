...
findings against code nobody will ship.
...
`pass`. A refutation is a claim you adjudicate, never a veto.
...
— blizzard builds no review machinery of its own. Exercise the change's end-to-end flows inside the chunk's environment,
where the services are available to drive.
...
**Anchor every finding at a file and line** (`<repo>/<path>:<line>`, or `<repo>/<path>::<symbol>`). The anchor is the
only stable handle the build can refute against and the only one you can match a refutation to on a later pass — a
finding that names no location can be neither acted on nor answered.
