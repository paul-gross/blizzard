...
Choose `novel` when the shortlist holds at least one candidate that survived the proposal-history check — something no
open, accepted, or still-holding passed proposal already answers.

Choose `nothing-new` only when the shortlist is empty and you are confident every candidate the survey recorded is
