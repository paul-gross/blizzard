...
This lane has no `pre-push` node, so the rebase-and-revalidate duty that recovery would normally carry lands here. For each repo still ahead: fetch, rebase onto the current base branch, and resolve any conflicts. Then re-run whatever checks the work item calls for, paying particular attention to whatever the conflict resolution touched — this node is the only station left that can revalidate it. Re-declare each rebased tip with `blizzard runner artifact commit` before declaring done again.
