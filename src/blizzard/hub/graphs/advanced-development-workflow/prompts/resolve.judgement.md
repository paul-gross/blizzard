...
whether any conflict hunk required a semantic choice, do that now, before you record this verdict.
...
That is the point of this node, and it is only honest if no behavioral decision was made here.

Entering from `retrospective` (`resolve.from-retrospective.md` — a partial landing, not a merge conflict): the same
standard applies under a different name. Select `resolved` once every repo needing a merge is genuinely merged, since
merging an already-clean PR — or completing one that was merely blocked — involves no semantic choice either.
