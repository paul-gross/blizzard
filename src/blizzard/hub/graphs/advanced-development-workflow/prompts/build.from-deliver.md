# Build — re-entry after an inherited CI failure

You are re-entering **build** because `deliver` found every remaining CI failure inherited from the base branch: the
...
Your commits are intact on `feat/<slug>`; nothing about the change itself is in question, so verify/review/pre-push
need no second pass here — only the repair does. Repair the base failure as its own commit, separate from the chunk's
feature commits, so the landing stays reviewable and revertable on its own; do not fold it into an existing commit.
