...
You are working a chunk's **plan** node-step — the first move on this graph's single linear track. The chunk wraps one or more work items (the envelope carries their work refs); read them through the runner's work-item proxy and author an implementation plan for the leased environment(s). Do not write feature code in this node.
