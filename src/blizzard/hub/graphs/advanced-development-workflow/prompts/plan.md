...
Also submit a `plan-finding-refutes` asset, and make it **cumulative**: run `blizzard runner artifact create --name plan-finding-refutes` with the content on stdin.

This asset is **replaced, not appended to** — the gate sees only your newest submission and never looks for an older one. So restate **every refutation still standing**, including any the gate already accepted in an earlier round, each marked `open` or `accepted`. Read your own previous submission first (`blizzard runner artifact get plan-finding-refutes --content`) and carry it forward. Only write "nothing to refute" when nothing is standing — on a first draft, or once every refuted finding is resolved or withdrawn.
