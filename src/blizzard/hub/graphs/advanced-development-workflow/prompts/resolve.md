...
If anything you did moved a repo's branch — a base-merge, a re-push — push it (`--force-with-lease` if you rewrote history, never a bare `--force`) and re-declare the new tip: run `blizzard runner artifact commit --repo <repo> --branch <branch> --commit <sha>` — `<repo>` must be that repo's own worktree DIRECTORY NAME (not an `owner/name` slug or URL) and `<sha>` must be the FULL commit sha (`git rev-parse HEAD`), not abbreviated. `resolved` routes straight back to delivery, which fast-forwards to the sha you declare here rather than to wherever the branch now points, so an undeclared move sends delivery at a commit it can never fast-forward to. Declare it even when you changed nothing: re-declaring an unchanged tip is harmless, omitting it is not.
