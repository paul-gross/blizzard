"""Runner routes — the fleet registry.

The fleet-registry surface: ``POST /runners`` registers a runner and
...
``POST /runners/{id}/heartbeats`` refreshes liveness; and
``POST /runners/{id}/pause`` / ``/resume`` set the operator's pause brake — the
