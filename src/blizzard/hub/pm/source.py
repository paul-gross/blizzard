"""The PM work-source seam (D-047/D-084/D-105/D-107) — a vendor-native pass-through read.
...
one instance per configured ``[[pm_source]]`` (D-105), pinned to its own repo and
carrying its own credentialed client.

D-107 grows the seam beyond ``fetch``: a binding also owns parsing its own ingest-token
...
:class:`IPmSourceRegistry` (D-105) replaces the single ``pm_source: IPmSource | None``
...
D-106 (Phase 2) adds ``owns``: ingest-time resolution and the board label/fetch both
...
class IPmSource(Protocol):
    """One configured, credentialed PM binding (D-047/D-105/D-107)."""
    ...
    def owns(self, pointer: PmPointer) -> bool:
        """True when ``pointer`` names this source's own configured repo (D-106).
        ...
        binding's own configured repo. This phase (D-104 not yet landed) the pointer
        carries no source name, so this is the only way to find a pointer's binding."""
        ...


class IPmSourceRegistry(Protocol):
    """The hub's configured PM sources, looked up by their declared ``name`` (D-105)."""
