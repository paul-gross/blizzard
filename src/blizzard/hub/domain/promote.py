...
from blizzard.hub.domain.work import IReadChunkRepository, IWriteChunkRepository


def tail_position(chunks: IReadChunkRepository) -> float:
    ...
    ready = chunks.list_ready()
    if not ready:
        return 0.0
    positions = chunks.queue_positions()
    promoted_ats = chunks.promoted_ats()
...
class PromoteService:
    ...
    def promote(self, chunk_id: str) -> int | None:
        ...
        if facts is not None and facts.promoted:
            return None
        tail = tail_position(self._chunks)
