"""Chunk restart — the operator's forced move of a chunk onto a node, now (issue #370).

An **event**, not the standing intent a migration edit records: it lands a ``chunk.restarted``
fact at a fresh epoch, which fences the running attempt out and re-aims the chunk. The in-flight
parks it consumes ride the same store write, so nothing survives to re-park the chunk at a node
...
class RestartNodeUnknown(Exception):
    """A restart named a node absent from the chunk's own graph (issue #370).

    Refused at request time: ``restart`` crosses no graph, so an unmatched name is a typo,
    never a landing to fall back from."""
...
class RestartService:
    ...
    def __init__(self, *, chunks: IWriteChunkRepository, clock: IClock, claim_lock: threading.Lock) -> None:
        self._chunks = chunks
        self._clock = clock
    ...
    def restart(self, chunk: Chunk, graph: Graph, *, node_name: str | None, by: str) -> int:
        """Move ``chunk`` onto ``node_name`` — its current node when unnamed — at a fresh epoch.

        Takes the resolved graph (``bzh:domain-takes-objects``). Raises for a terminal chunk or
        a node the graph does not carry, writing nothing either way; returns the fresh
        ``chunk_restarts.id`` (issue #213's activity-feed key)."""
        with self._claim_lock:
            return self._restart_locked(chunk, graph, node_name=node_name, by=by)

    def _restart_locked(self, chunk: Chunk, graph: Graph, *, node_name: str | None, by: str) -> int:
        ...
        target = self._target(graph, from_node_id, node_name)
    ...
    @staticmethod
    def _target(graph: Graph, from_node_id: str | None, node_name: str | None) -> Node:
        """The node the move lands on: the named one, else the chunk's current node.

        A chunk that has not moved stands on nowhere, so the entry node — where it would have
        started — is the one default. One that HAS moved, onto a node this graph does not carry,
        is refused instead of rewound to the entry."""
        if node_name is not None:
            named = graph.node_by_name(node_name)
            if named is None:
                raise RestartNodeUnknown(node_name, graph.graph_id)
            return named
        if from_node_id is None:
            entry = graph.node_by_id(graph.entry_node_id)
            assert entry is not None, f"graph {graph.graph_id} names an entry node it does not carry"
            return entry
