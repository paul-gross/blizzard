"""Chunk detach — the operator's forcible release of a chunk from its runner (D-088).
...
runner learns of the release on its own next tick and closes the lease (D-088; the
...
runner is released, but the escalation stays open until a human requeues it (D-067).
...
rules are scoped to D-045/D-032/D-067).
...
class DetachService:
    """Release a chunk from its runner without touching any escalation — ``blizzard hub detach`` (D-088)."""
    ...
    def detach(self, chunk: Chunk) -> None:
        """Release the chunk's live route so it re-derives ``ready`` (D-088).
