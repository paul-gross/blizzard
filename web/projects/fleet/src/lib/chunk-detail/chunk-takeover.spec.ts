...
describe('ChunkTakeover', () => {
  ...
  it('renders the raw field as guidance prose, with no command box or copy button, when unwrapped', async () => {
    ...
    expect(el.querySelector('[data-testid="copy-takeover"]')).toBeNull();
    expect(el.querySelector('[data-testid="takeover-command-raw-fallback"]')).toBeNull();
    expect(el.querySelector('[data-testid="escalation-guidance"]')?.textContent).toContain(
