 ...
 * arrow) is dropped rather than rendered as placeholder junk (issue #212 AC 5), so a
 * frame carrying neither node degrades to exactly today's `C-1NWW → running`. `runner`
 * is the compact runner ref when the frame names one, else omitted — an unclaimed
 * transition (e.g. a promote or a stop past the point the route released) renders no
 * runner line at all rather than an empty one.
 *
