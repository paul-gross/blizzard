...
describe('GraphDetailHeader', () => {
  async function mount(inputs: {
    ...
    fixture.componentRef.setInput('name', inputs.name ?? 'build');
  ...
  it('renders the identity, entry-node line, enabled badge, and a Retire button for a non-retired graph', async () => {
  ...
  it('withholds the retire/enable controls for a contributor (no graph:edit, #93), keeping the identity and badge', async () => {
