...
const LAID_OUT: LaidOutGraph = {
  ...
  edges: [
    { id: 'e0', kind: 'advance', path: '', label: null, fromNodeId: 'n_a', target: { kind: 'node', nodeId: 'n_b' }, choiceId: 'c_pass' },
    { id: 'e1', kind: 'advance', path: '', label: null, fromNodeId: 'n_b', target: { kind: 'node', nodeId: 'n_c' }, choiceId: 'c_pass2' },
    { id: 'e2', kind: 'advance', path: '', label: null, fromNodeId: 'n_c', target: { kind: 'done' }, choiceId: 'c_done' },
  ],
...
describe('incidentEdgeIds', () => {
  ...
  it('finds a node with only an incoming edge', () => {
    const withSink: LaidOutGraph = {
      ...
      edges: [
        ...LAID_OUT.edges,
        { id: 'e4', kind: 'advance', path: '', label: null, fromNodeId: 'n_c', target: { kind: 'node', nodeId: 'n_d' }, choiceId: 'c_x' },
      ],
    };
...
describe('endpointNodeIds', () => {
  ...
  it('resolves both endpoints of a regular edge selection', () => {
    expect(
      endpointNodeIds({ kind: 'edge', edgeId: 'e0', fromNodeId: 'n_a', target: { kind: 'node', nodeId: 'n_b' }, choiceId: 'c_pass', edgeKind: 'advance' }),
    ).toEqual(['n_a', 'n_b']);
  });

  it('resolves only the source for an edge into the done terminal', () => {
    expect(
      endpointNodeIds({ kind: 'edge', edgeId: 'e2', fromNodeId: 'n_c', target: { kind: 'done' }, choiceId: 'c_done', edgeKind: 'advance' }),
    ).toEqual(['n_c']);
  });

  it('resolves a single node for a self-loop selection', () => {
    expect(
      endpointNodeIds({ kind: 'edge', edgeId: 'e3', fromNodeId: 'n_b', target: { kind: 'node', nodeId: 'n_b' }, choiceId: 'c_fail', edgeKind: 'retry' }),
    ).toEqual(['n_b']);
  });

  it('resolves only the source for a migration edge (no second node in this graph)', () => {
    expect(
      endpointNodeIds({
        kind: 'edge',
        edgeId: 'e5',
...
describe('resolveSelectedChoice', () => {
  ...
  it('resolves an edge into done with a done-kind target', () => {
    const resolved = resolveSelectedChoice(GRAPH, {
      kind: 'edge',
      edgeId: 'e2',
      fromNodeId: 'n_c',
    ...
    expect(resolved?.target).toEqual({ kind: 'done' });
    expect(resolved?.name).toBe('landed');
  });

  it('resolves a migration edge with a graph-kind target carrying the target graph name', () => {
    const resolved = resolveSelectedChoice(GRAPH, {
      kind: 'edge',
      edgeId: 'e5',
      fromNodeId: 'n_a',
    ...
    expect(resolved?.target).toEqual({ kind: 'graph', targetGraph: 'bas-dwf' });
  });
